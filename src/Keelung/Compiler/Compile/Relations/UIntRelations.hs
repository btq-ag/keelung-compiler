...
lookup var relations = case Map.lookup var (toRoot relations) of
  ...
  Just (RootIs rotation parent) -> case lookup parent relations of
    Root -> RotateOf rotation parent
    Value value -> Value $ Arith.bitWiseRotateL (widthOf var) rotation value
    RotateOf rotation' grandparent -> RotateOf ((rotation + rotation') `mod` widthOf var) grandparent
...
assertEqual x y xs = Just <$> relate x 0 y xs
    ...
    EQ -> case compare (childrenSizeOf a) (childrenSizeOf b) of
      ...
      where
        ...
        childrenSizeOf ref = case lookup ref relations of
          ...
          Value _ -> 0
          RotateOf _ parent -> childrenSizeOf parent
...
relationBetween var1 var2 xs = case (lookup var1 xs, lookup var2 xs) of
  ...
  (Root, RotateOf rotation2 parent2) ->
    if var1 == parent2
  ...
  (RotateOf rotation1 parent1, Root) ->
    if var2 == parent1
  ...
  (RotateOf rotation1 parent1, RotateOf rotation2 parent2) ->
    if parent1 == parent2
      then Just ((rotation1 - rotation2) `mod` widthOf var1)
      else Nothing
  (Value _, RotateOf _ _) -> Nothing -- var1 is a constant value
  (RotateOf _ _, Value _) -> Nothing -- var2 is a constant value
...
--   Nothing -> Root
--   Just (Constant value) -> Value value
--   Just (RootIs rotation parent) -> RotateOf rotation parent
