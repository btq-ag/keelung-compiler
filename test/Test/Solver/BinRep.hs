...
genPoly :: Integral n => Int -> [Bool] -> Gen (Poly n, IntMap.IntMap (Bool, Bool))
genPoly fieldBitWidth signOptions = do
  -- there can be atmost `fieldBitWidth` number of coefficients
  signs <- vectorOf fieldBitWidth $ do
    sign <- elements signOptions
    assignment <- elements [True, False]
    return (sign, assignment)

  -- remove some of the coefficients
  numberOfCoeffs <- choose (1, fieldBitWidth) -- keep at least 1 coefficient
  filtered <- take numberOfCoeffs <$> shuffle (zip [0 ..] signs)

  let (coeffs, constant) =
        foldl
          ( \(poly, c) (power, (sign, assignment)) ->
              let coeff = if sign then 2 ^ power else -(2 ^ power)
               in ((power, coeff) : poly, c + (if assignment then 1 else 0) * coeff)
          )
          ([], 0)
          filtered

  let polynomial = case Poly.buildEither (-constant) coeffs of
        Left _ -> error "Poly.buildEither"
        Right p -> p

  -- [(var, (sign, assignment))]
  let assignments = IntMap.fromList filtered
  return (polynomial, assignments)

tests :: SpecWith ()
tests = describe "BinRep Detection" $ do
  it "`computeBinPoly` : all positvie / mod 11" $ do
    forAll (genPoly 3 [True]) $ \(polynomial :: Poly (Prime 11), assignments) -> do
      -- testing with [(var, sign)] pairs
      let actual = computeBinPoly 3 (const True) polynomial
      let expected = Just (BinPoly $ map swap $ IntMap.toList $ fmap fst assignments)
      actual `shouldBe` expected

  it "`computeBinPoly` : mixed signs / mod 11" $ do
    forAll (genPoly 3 [True, False]) $ \(polynomial :: Poly (Prime 11), assignments) -> do
      -- testing with [(var, sign)] pairs
      let actual = computeBinPoly 3 (const True) polynomial
      let expected = Just (BinPoly $ map swap $ IntMap.toList $ fmap fst assignments)
      actual `shouldBe` expected

  it "`computeBinPoly` : mixed signs / mod GF181" $ do
    forAll (genPoly 180 [True, False]) $ \(polynomial :: Poly GF181, assignments) -> do
      -- testing with [(var, sign)] pairs
      let actual = computeBinPoly 180 (const True) polynomial
      let expected = Just (BinPoly $ map swap $ IntMap.toList $ fmap fst assignments)
      actual `shouldBe` expected
