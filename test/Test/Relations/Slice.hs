  ...
  describe "SliceRelations.assign" $ do
    it "should return the assigned value on lookup" $ do
      ...
      forAll genParam $ \(ref, interval, val) -> do
        let expected = Slice.normalize $ Slice.Slice ref (fst interval) (IntMap.singleton (fst interval) (Slice.Constant val)) -- Slice.mapInterval (const (Slice.Constant val)) interval (Slice.fromRefU ref)
