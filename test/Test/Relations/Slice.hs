  ...
  describe "SliceRelations.assign" $ do
    it "should return the assigned value on lookup" $ do
      ...
      forAll genParam $ \(ref, interval, val) -> do
        let expected = SliceLookup.normalize $ SliceLookup.SliceLookup ref (fst interval) (IntMap.singleton (fst interval) (SliceLookup.Constant val)) -- SliceLookup.mapInterval (const (SliceLookup.Constant val)) interval (SliceLookup.fromRefU ref)
