...
tests = describe "Limb Bound Calculation" $ do
  describe "Instances" $ do
    it "0" $ do
      let limbs = (0, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True])
      uncurry calculateBounds limbs `shouldBe` (0, 6)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(True, 1)]

    it "1" $ do
      let limbs = (3, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True])
      uncurry calculateBounds limbs `shouldBe` (3, 9)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(True, 2)]

    it "2" $ do
      let limbs = (0, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 False])
      uncurry calculateBounds limbs `shouldBe` (-3, 3)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(False, 1)]

    it "3" $ do
      let limbs = (1, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True, Limb.CarryLimb (RefUX 2 0) (Seq.fromList [(False, 1), (True, 1)])])
      uncurry calculateBounds limbs `shouldBe` (0, 9)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(True, 2)]

    it "4" $ do
      let limbs = (3, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 False, Limb.CarryLimb (RefUX 2 0) (Seq.fromList [(False, 1), (True, 1)])])
      uncurry calculateBounds limbs `shouldBe` (-1, 11)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(False, 1), (True, 1)]

    it "5" $ do
      let limbs = (3, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 False, Limb.newOperand (RefUX 2 0) 2 0 False, Limb.newOperand (RefUX 2 0) 2 0 False, Limb.CarryLimb (RefUX 2 0) (Seq.fromList [(False, 1), (True, 1)])])
    ...
    it "7" $ do
      let limbs = (0, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 False, Limb.newOperand (RefUX 2 0) 2 0 False])
      uncurry calculateBounds limbs `shouldBe` (-6, 0)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(True, 1), (False, 1)]

    it "8" $ do
      let limbs = (0, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 False])
      uncurry calculateBounds limbs `shouldBe` (-3, 9)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(False, 1), (True, 1)]
    ...
    it "10" $ do
      let limbs = (0, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True, Limb.CarryLimb (RefUX 2 0) (Seq.fromList [(False, 1), (True, 1)])])
      uncurry calculateBounds limbs `shouldBe` (-1, 11)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(False, 1), (True, 1)]

    it "11" $ do
      let limbs = (0, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 True, Limb.newOperand (RefUX 2 0) 2 0 False, Limb.newOperand (RefUX 2 0) 2 0 False])
      uncurry calculateBounds limbs `shouldBe` (-6, 6)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(True, 1), (False, 1)]

    it "12" $ do
      let limbs = (3, Seq.fromList [Limb.newOperand (RefUX 2 0) 2 0 False, Limb.newOperand (RefUX 2 0) 2 0 False, Limb.newOperand (RefUX 2 0) 2 0 True])
      uncurry calculateBounds limbs `shouldBe` (-3, 6)
      uncurry (calculateCarrySigns 2) limbs `shouldBe` Seq.fromList [(False, 1), (True, 1)]

    it "13" $ do
      let limbs = (0, Seq.fromList [Limb.newOperand (RefUX 180 0) 178 0 True, Limb.newOperand (RefUX 180 0) 178 0 True])
  ...
  describe "`calculateSignsOfLimbs`" $ it "should make non-carry bits positive" $ do
    let genLimbs = do
          ...
          let limbs = map (Limb.newOperand refU width 0) signs
  ...
  describe "one special cases of `calculateCarrySigns`" $ it "1 + 2bit - 2bit - 2bit - 2bit" $ do
    let limbs =
          Seq.fromList
            [ Limb.newOperand (RefUX 2 0) 2 0 True,
              Limb.newOperand (RefUX 2 0) 2 0 False,
              Limb.newOperand (RefUX 2 0) 2 0 False,
              Limb.newOperand (RefUX 2 0) 2 0 False
            ]
