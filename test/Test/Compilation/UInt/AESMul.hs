  ...
  describe "AES GF(256) Multiplication" $ do
    it "Constants only" $ do
      ...
      property $ \(x :: Word8, y :: Word8) -> do
        ...
        forM_ [gf181, Prime 5] $ \field -> testCompiler field (program (fromIntegral x) (fromIntegral y)) [] [] expected
    ...
    it "1 variable / 1 constant" $ do
      ...
      property $ \(x :: Word8, constant :: Word8) -> do
        ...
        forM_ [gf181, Prime 17] $ \field -> testCompiler field (program (fromIntegral constant)) [toInteger x] [] expected
    ...
    it "2 byte variables" $ do
      ...
      property $ \(x :: Word8, y :: Word8) -> do
        let expected = [toInteger (U.aesMul (U.new 8 (toInteger x)) (U.new 8 (toInteger y)))]
        forM_ [gf181, Prime 17] $ \field -> testCompiler field program (map toInteger [x, y]) [] expected
    ...
    it "2 variables / 1 constant" $ do
      ...
      property $ \(x, y :: Word8, c :: Word8) -> do
        ...
        forM_ [gf181, Prime 17] $ \field -> testCompiler field (program (fromIntegral c)) (map toInteger [x, y]) [] expected
    ...
    it "3 variables / 1 constant" $ do
      ...
      property $ \(x, y, z :: Word8, c :: Word8) -> do
        ...
        forM_ [gf181, Prime 17] $ \field -> testCompiler field (program (fromIntegral c)) (map toInteger [x, y, z]) [] expected
