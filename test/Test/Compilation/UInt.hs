  ...
  describe "Unsigned Integers" $ do
    ...
    describe "Big Int I/O" $ do
      it "10 bit / GF257" $ do
        ...
        check (Prime 257) program [300] [] [300]
    ...
    describe "Conditionals" $ do
      it "variable / variable / variable" $ do
        ...
        forAll genParam $ \(p, x, y) -> do
          ...
          check gf181 program [if p then 1 else 0, fromIntegral x, fromIntegral y] [] expected
      ...
      it "variable / variable / constant" $ do
        ...
        forAll genParam $ \(p, x, y) -> do
          let expected = [fromIntegral $ if p then x else y]
          check gf181 (program (fromIntegral y)) [if p then 1 else 0, fromIntegral x] [] expected
      ...
      it "variable / constant / variable" $ do
        ...
        forAll genParam $ \(p, x, y) -> do
          let expected = [fromIntegral $ if p then x else y]
          check gf181 (program (fromIntegral x)) [if p then 1 else 0, fromIntegral y] [] expected
      ...
      it "variable / constant / constant" $ do
        ...
        forAll genParam $ \(p, x, y) -> do
          let expected = [fromIntegral $ if p then x else y]
          check gf181 (program (fromIntegral x :: UInt 4) (fromIntegral y)) [if p then 1 else 0] [] expected

      it "constant predicate" $ do
        let program = do
              return $ cond true (3 :: UInt 2) 2
        check gf181 program [] [] [3]
    ...
    describe "Equalities" $ do
      it "variable / constant" $ do
        ...
        forAll (choose (0, 15)) $ \x -> do
          ...
          check gf181 program [x] [] expected
          check (Prime 13) program [x] [] expected
          check (Binary 7) program [x] [] expected
      ...
      it "variables (Prime 13)" $ do
        ...
        forAll genPair $ \(x, y) -> do
          ...
          check (Prime 13) program [x, y] [] expected
          check (Binary 7) program [x, y] [] expected
      ...
      it "neq: variable / constant" $ do
        ...
        forAll (choose (0, 15)) $ \x -> do
          ...
          check (Prime 13) program [x `mod` 16] [] expected
      ...
      it "neq (40 bits / Prime 13)" $ do
        ...
        check (Prime 13) program [12345, 12344] [] [1]
        check (Prime 13) program [12340000001, 12340000000] [] [1]
        check (Prime 13) program [1234, 1234] [] [0]
    ...
    describe "Slice" $ do
      it "should throw exception when the starting index < 0" $ do
        ...
        check (Binary 7) program [0] [] [0] `shouldThrow` anyException
        check (Prime 17) program [0] [] [0] `shouldThrow` anyException
        check gf181 program [0] [] [0] `shouldThrow` anyException
    ...
    describe "Join" $ do
      it "constant (slice width: 4 / 4)" $ do
        ...
        forAll genParam $ \(x, y) -> do
          ...
          check (Binary 7) (program x y) [] [] expected
          check (Prime 17) (program x y) [] [] expected
          check gf181 (program x y) [] [] expected
