...
tests = describe "Statement" $ do
  describe "toField" $ do
    describe "from variable" $ do
      ...
      it "GF181" $ do
        forAll (chooseInteger (-100, 511)) $ \n -> do
          check gf181 program [n] [] [n `mod` 256]
      it "Prime 2" $ do
    ...
    describe "from constant" $ do
      ...
      it "GF181" $ do
        forAll (chooseInteger (-100, 511)) $ \n -> do
          check gf181 (program (fromInteger n)) [] [] [n `mod` 256]
      it "Prime 2" $ do
        forAll (chooseInteger (-10, 4)) $ \n -> do
          check (Prime 2) (program (fromInteger n)) [] [] [n `mod` 2]
      it "Binary 7" $ do
        forAll (chooseInteger (-10, 8)) $ \n -> do
          check (Binary 7) (program (fromInteger n)) [] [] [n `mod` 4]
    ...
    it "from variables" $ do
      ...
      property $ \(x :: Word) -> do
        ...
        forM_ [gf181, Prime 2, Binary 7] $ \field -> do
          check field program bits [] [fromIntegral x]
    ...
    it "from Field element" $ do
      ...
      property $ \(x :: Word) -> do
        ...
        forM_ [gf181, Prime 5, Binary 7] $ \field -> do
          check field program [fromIntegral (x `mod` 4)] [] [fromIntegral expected]