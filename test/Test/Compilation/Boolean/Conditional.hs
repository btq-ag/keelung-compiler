...
tests = describe "conditional" $ do
  it "constant predicate / constant branches" $ do
    ...
    forM_ [gf181, Prime 2, Binary 7] $ \field -> do
      runAll field (program false false false) [] [] [0]
      runAll field (program false false true) [] [] [1]
      runAll field (program false true false) [] [] [0]
      runAll field (program false true true) [] [] [1]
      runAll field (program true false false) [] [] [0]
      runAll field (program true false true) [] [] [0]
      runAll field (program true true false) [] [] [1]
      runAll field (program true true true) [] [] [1]
