...
instance (GaloisField n, Integral n, Show var) => Show (Relate (Field.UnionFind n) var n) where
  show (RelateVarField var1 var2 (slope, intercept)) = "RelateVarField " <> show var1 <> " " <> show var2 <> " (" <> show slope <> ", " <> show intercept <> ")"
...
instance (Arbitrary n, GaloisField n, Integral n, Arbitrary var) => Arbitrary (Relate (Field.UnionFind n) var n) where
  arbitrary =
    RelateVarField
      <$> arbitrary
      <*> arbitrary
...
instance {-# OVERLAPPING #-} (Arbitrary n, GaloisField n, Integral n) => Arbitrary (Relate (Field.UnionFind n) Var n) where
  arbitrary =
    RelateVarField
      <$> chooseInt (0, 100)
      <*> chooseInt (0, 100)
...
instance (Arbitrary var) => Arbitrary (Relate Boolean.UnionFind var Bool) where
  arbitrary =
    RelateVarBool
      <$> arbitrary
      <*> arbitrary
...
instance {-# OVERLAPPING #-} Arbitrary (Relate Boolean.UnionFind Var Bool) where
  arbitrary =
    RelateVarBool
      <$> chooseInt (0, 100)
      <*> chooseInt (0, 100)
