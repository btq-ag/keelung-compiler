...
import Keelung.Data.Reference (Ref (..), RefF (..), RefB (..))
...
tests = describe "UnionFind" $ do
  ...
  it "error" $ do
    ...
    let relates = [RelateRelations (F (RefFX 0)) (B (RefBP 0)) (1078936104348021732734147004977251905910385367168331932, 175045228176580001514165878868385845338554771781719336), RelateRelations (F (RefFX 0)) (B (RefBI 5)) (105946717925123061829485584721240966793283594296367397, 476199024211118671754238663712601041772701772451390232)]
    let xs = foldl applyRelate (Relations.new (Options.new Field.gf181)) (relates :: [Relate (Relations.Relations GF181) Ref GF181]) :: Relations.Relations GF181
    print xs
